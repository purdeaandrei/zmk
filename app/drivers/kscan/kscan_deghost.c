...
#define EXISTS(row, col) ((config->transform_filled_len == 0) ? 1 : (                           \
                          (config->transform_filled_len <= (row) * config->cols + (col)) ? 0 :  \
                          (config->transform_filled[(row) * config->cols + (col)])))

static void kscan_deghost_callback_for_child(const struct device *deghost_dev,
                                            uint32_t row,
                                            uint32_t col,
                                            bool pressed) {
            ...
            for (uint32_t ocol=0; ocol<config->cols; ocol++) {
                ...
                if (pressed_in_rectangle == 2) {
                    if (WAS_KEY_STATUS_GHOSTING_BEFORE(KEY(row, ocol))) {
                        ...
                        KEY(row, ocol) |= KEY_STATUS_FORCE_NOT_RECHECK; // optimization, next WAS_KEY_STATUS_GHOSTING_BEFORE() call will be false
                    }
...
static int kscan_deghost_init(const struct device *dev) {
    return 0;
}
...
#define _TRANSFORM_ENTRY(i, transform_node)                                       \
     [(KT_ROW(DT_PROP_BY_IDX(transform_node, map, i)) *                           \
         DT_PROP(transform_node, columns)) +                                      \
         KT_COL(DT_PROP_BY_IDX(transform_node, map, i))] = 1

#define KSCAN_DEGHOST_INIT(n)                                                                               \
    static const uint8_t transform_filled_##n[] = {COND_CODE_1(DT_INST_NODE_HAS_PROP(n, transform),         \
                                                (LISTIFY(DT_PROP_LEN(DT_INST_PHANDLE(n, transform), map),   \
                                                 _TRANSFORM_ENTRY, (, ), DT_INST_PHANDLE(n, transform))),   \
                                                ())};                                                       \
    static void kscan_deghost_callback_for_child_##n(const struct device *child_dev, uint32_t row,          \
        ...
        kscan_deghost_callback_for_child(deghost_dev, row, column, pressed)      ;                          \
                                                                                                            ...
                                                                                                            \
    static uint8_t key_status_##n                                                                           \
               [DT_INST_PROP_OR(n, rows,                                                                    \
                DT_PROP_OR(DT_INST_PHANDLE(n, transform), rows,                                             \
                DT_PROP_LEN_OR(DT_INST_PHANDLE(n, kscan), row_gpios,                                        \
                ZMK_MATRIX_ROWS)))]                                                                         \
               [DT_INST_PROP_OR(n, columns,                                                                 \
                DT_PROP_OR(DT_INST_PHANDLE(n, transform), columns,                                          \
                DT_PROP_LEN_OR(DT_INST_PHANDLE(n, kscan), col_gpios,                                        \
                ZMK_MATRIX_COLS)))];                                                                        \
                                                                                                            \
    ...
    static struct kscan_deghost_data kscan_deghost_data_##n = {                                             \
        .key_status = &key_status_##n[0][0],                                                                \
                                                                                                            ...
                                                                                                            \
    DEVICE_DT_INST_DEFINE(n, kscan_deghost_init, NULL, &kscan_deghost_data_##n, &kscan_deghost_config_##n,  \
                      APPLICATION, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &deghost_driver_api);
